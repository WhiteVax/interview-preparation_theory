...
+ Описание: Метод poll() извлекает и возвращает элемент из головы очереди. Если очередь пуста, он возвращает null, не выбрасывая исключения.
  + Поведение при пустой очереди: Если очередь пуста, метод возвращает null.
    + Применение: Этот метод часто используется, когда важно избежать выбрасывания исключений при пустой очереди.
 
Метод remove()
    
+ Описание: Метод remove() также извлекает и возвращает элемент из головы очереди. Однако, если очередь пуста, он выбрасывает исключение NoSuchElementException.
  + Поведение при пустой очереди: Если очередь пуста, метод выбрасывает исключение NoSuchElementException.
    + Применение: Этот метод используется, когда необходимо обеспечить наличие элементов в очереди и обработку ошибок, если очередь пуста.
### 28. В чем отличие метода element() от peek()?
Метод peek()

+ Описание: Метод peek() возвращает первый элемент из очереди, не удаляя его, и возвращает null, если очередь пуста.
  + Поведение при пустой очереди: Если очередь пуста, метод возвращает null и не выбрасывает исключения.
    + Применение: Используется, когда нужно просто просмотреть первый элемент очереди без его удаления, и при этом не нужно выбрасывать исключения, если очередь пуста.

Метод element()

+ Описание: Метод element() также возвращает первый элемент из очереди, не удаляя его, но в отличие от peek(), он выбрасывает исключение NoSuchElementException, если очередь пуста.
  + Поведение при пустой очереди: Если очередь пуста, метод выбрасывает исключение NoSuchElementException.
    + Применение: Используется, когда нужно гарантировать, что элемент в очереди существует, и в случае пустой очереди необходимо обработать ошибку.
### 29. В чем отличие метода element() от poll()?
Метод element()

+ Описание: Метод element() возвращает первый элемент из очереди, но не удаляет его.
  + Поведение при пустой очереди: Если очередь пуста, метод выбрасывает исключение NoSuchElementException.
    + Применение: Используется, когда вы хотите просмотреть первый элемент очереди, но при этом важно, чтобы очередь не была пуста, так как в противном случае выбрасывается исключение.

Метод poll()

+ Описание: Метод poll() также извлекает первый элемент из очереди, но в отличие от element(), он удаляет этот элемент из очереди.
  + Поведение при пустой очереди: Если очередь пуста, метод возвращает null и не выбрасывает исключения.
    + Применение: Используется, когда нужно извлечь и удалить первый элемент из очереди, и при этом необходимо избежать выбрасывания исключений, если очередь пуста.
### 30. Перечислите наиболее часто используемые реализации интерфейса Queue.

| Реализация                | Интерфейсы                | Тип                   | Краткое описание                                                                            |
| ------------------------- | ------------------------- | --------------------- | ------------------------------------------------------------------------------------------- |
| **LinkedList**            | `List`, `Deque`, `Queue`  | FIFO / Deque          | Универсальная реализация списка и очереди. Поддерживает добавление/удаление с обоих концов. |
| **PriorityQueue**         | `Queue`                   | Очередь с приоритетом | Извлекает элементы по приоритету (natural ordering или `Comparator`), не FIFO.              |
| **ArrayDeque**            | `Deque`, `Queue`          | Deque                 | Высокопроизводительная очередь на массиве. Быстрее `LinkedList` в большинстве случаев.      |
| **DelayQueue**            | `BlockingQueue` → `Queue` | Blocking Queue        | Элементы доступны только после истечения задержки. Используется в планировщиках.            |
| **SynchronousQueue**      | `BlockingQueue` → `Queue` | Blocking Queue        | Не хранит элементы — каждый `put` ждёт `take`. Для передачи данных между потоками.          |
| **ConcurrentLinkedQueue** | `Queue`                   | Concurrent Queue      | Неблокирующая очередь на CAS, подходит для высоконагруженных многопоточных систем.          |

### 31. Что такое ограниченные и неограниченные очереди?
*Ограниченные очереди* (Bounded Queues, когда нужно контролировать ограничение по памяти)

Ограниченная очередь — это очередь, размер которой ограничен заранее определённой максимальной вместимостью. Когда очередь заполняется до этой предельной величины, попытки добавить новые элементы могут либо блокировать выполнение, либо завершаться с ошибкой, в зависимости от реализации.
+ Особенности:
  + Размер очереди фиксирован и не может быть увеличен в процессе работы.
  + Когда очередь заполнена, попытка добавить новый элемент может либо блокировать текущий поток до освобождения места (например, в случае с BlockingQueue), либо вызвать исключение (например, в случае с ArrayBlockingQueue).
  + Такие очереди полезны в случаях, когда необходимо ограничить использование памяти или когда нужно контролировать количество элементов в системе.
    + Примеры:
      + ArrayBlockingQueue — ограниченная очередь с фиксированным размером (при превышении размера выкидывает исключение).
      + LinkedBlockingQueue (может быть как ограниченной, так и неограниченной в зависимости от конструктора).
      + PriorityBlockingQueue (может быть ограниченной).

*Неограниченные очереди* (Unbounded Queues)

Неограниченная очередь — это очередь, которая не имеет заранее заданного лимита на количество элементов. Элементы могут добавляться в очередь без каких-либо ограничений, пока не исчерпается доступная память системы.

+ Особенности:
  + Размер очереди не ограничен, элементы могут добавляться до тех пор, пока система не исчерпает ресурсы памяти.
  + Неограниченные очереди обычно не блокируют потоки при добавлении элементов, если очередь не заполнена.
  + Такие очереди полезны в случаях, когда нужно обеспечить неограниченную буферизацию данных или когда размер очереди не важен.
    + Примеры:
      + LinkedList — реализует неограниченную очередь, так как размер зависит от доступной памяти.
      + ConcurrentLinkedQueue — неограниченная очередь с неблокирующими операциями.
      + PriorityQueue — неограниченная очередь с приоритетами.
### 32. Что такой односторонние и двусторонние очереди?
Односторонняя очередь (или просто очередь) — это структура данных, которая реализует принцип FIFO (First In, First Out — "первым пришел, первым ушел"). В таких очередях элементы добавляются в конец (с помощью операции offer() или add()) и извлекаются с начала (с помощью операции poll() или remove()).

+ Особенности:
  + Элементы обрабатываются в том порядке, в котором они были добавлены в очередь.
  + Извлечение элементов происходит только с одного конца (с начала очереди).
  + Используется для реализации стандартных очередей, таких как очередь задач или потоков, где важно обрабатывать элементы в порядке их поступления.
    +	Примеры:
      +	LinkedList (при использовании как обычная очередь).
      +	ArrayBlockingQueue.
      +	PriorityQueue (очередь с приоритетами, но элементы извлекаются не по порядку добавления, а по приоритету).

Двусторонняя очередь (или Deque — от английского Double Ended Queue) — это очередь, которая позволяет вставлять и извлекать элементы с обоих концов, как с начала, так и с конца. В таких очередях можно добавлять или извлекать элементы с обеих сторон, что дает большую гибкость при работе с элементами.
+ Особенности:
+ В отличие от односторонней очереди, в двусторонней очереди элементы могут добавляться или извлекаться как с начала, так и с конца очереди.
    + Поддерживает операции как для обычной очереди (FIFO), так и для стека (LIFO — Last In, First Out), что делает её более универсальной.
    + Очень полезна для реализации различных алгоритмов, например, поиска в ширину или в глубину, работы с окнами и буферами.
      + Примеры:
        + ArrayDeque — реализует интерфейс Deque и предоставляет эффективное добавление/удаление элементов с обоих концов.
        + LinkedList — также реализует интерфейс Deque и поддерживает операции с обоих концов.
    
Основные различия между односторонними и двусторонними очередями

| Характеристика      | Односторонняя очередь | Двусторонняя очередь (Deque) |
| ------------------- | --------------------- | ---------------------------- |
| Тип доступа         | Только с начала       | С начала и с конца           |
| Принцип             | FIFO                  | FIFO или LIFO                |
| Добавление          | В конец               | В начало и в конец           |
| Извлечение          | С начала              | С начала и с конца           |
| Типичное применение | Очереди задач, потоки | Стек, очереди, буферы        |

### 33. В чём отличие Deque от Queue?
Queue — это интерфейс, который представляет одностороннюю очередь, которая поддерживает добавление элементов в конец и извлечение элементов из начала (по принципу FIFO — First In, First Out).

Deque (от Double Ended Queue) — это расширение интерфейса Queue, которое представляет двустороннюю очередь, позволяющую добавлять и извлекать элементы как с начала, так и с конца очереди.

*Операции и возможности*

Queue:
+ Поддерживает только операции для работы с одним концом очереди:
  + offer(E e) — добавление элемента в конец очереди.
  + poll() — извлечение элемента из начала очереди (с удалением).
  + peek() — просмотр первого элемента очереди без его удаления.
  
Deque:
+ Поддерживает все операции, доступные в Queue, и дополнительные операции для работы с обоими концами очереди:
  + addFirst(E e) — добавление элемента в начало очереди.
  + addLast(E e) — добавление элемента в конец очереди.
  + removeFirst() — извлечение элемента с начала очереди.
  + removeLast() — извлечение элемента с конца очереди.
  + peekFirst() — просмотр первого элемента очереди.
  + peekLast() — просмотр последнего элемента очереди.

### 34. В чём отличие методов removeLast() и pollLast()?
removeLast()

+ Описание: Метод removeLast() удаляет последний элемент из двусторонней очереди. Если очередь пуста, он выбрасывает исключение NoSuchElementException.
+ Поведение при пустой очереди: Если очередь пуста, вызов этого метода приведет к выбросу исключения.

pollLast()

+ Описание: Метод pollLast() также удаляет последний элемент из двусторонней очереди. Однако, если очередь пуста, он не выбрасывает исключение, а вместо этого возвращает null.
+ Поведение при пустой очереди: Если очередь пуста, метод просто возвращает null, не выбрасывая исключения.
### 35. Назовите самую распространенную реализацию Deque.
LinkedList/ ArrayDeque

ArrayDeque

+ Описание: ArrayDeque является эффективной реализацией двусторонней очереди (deque), которая использует динамический массив для хранения элементов. Она предоставляет операции для добавления и извлечения элементов с обоих концов очереди (как с начала, так и с конца).

Особенности:

+ Очень эффективна для большинства операций (например, добавление и удаление элементов с обоих концов).
+ Меньше накладных расходов по сравнению с LinkedList, так как использует массив, а не связанные узлы.
+ Не поддерживает null элементы.
+ Вместо LinkedList рекомендуется использовать ArrayDeque для реализации двусторонней очереди, если не требуется специфической функциональности связанных списков.

Почему ArrayDeque является наиболее распространённой реализацией?

1.	Эффективность: Он обеспечивает высокую производительность для большинства операций, таких как добавление и извлечение элементов с обоих концов.
2.	Простота использования: Это простая и удобная реализация, которая лучше всего подходит для реализации двусторонних очередей без необходимости в дополнительных ресурсах, таких как ссылки в LinkedList.
3.	Меньше накладных расходов: В отличие от LinkedList, которая требует дополнительных ресурсов для хранения ссылок на предыдущий и следующий элементы, ArrayDeque использует массив, что делает его более компактным и быстрым для большинства операций.
      Таким образом, ArrayDeque — это стандартный выбор для реализации двусторонних очередей в Java, если не требуется специфическое поведение, предоставляемое другими реализациями.
